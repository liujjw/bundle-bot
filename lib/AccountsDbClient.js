...
class AccountsDbClient {
    ...
    async setCompoundParams() {
        ...
        this.client.set("liquidationBonus", liquidationBonus);
        this.client.set("closeFactor", closeFactor);
        this.client.set("collateralFactors", collateralFactors);
        this.client.set("prices", prices);
        this.client.set("exchangeRates", exchangeRates);
    }
    ...
    async fetchBatchOfAccounts(last, blockNumber) {
        ...
        if(this.blockNumber !== undefined) {
            queryString = `query {
                accounts(block: {number: ${this.blockNumber}}, first: 1000, where: { hasBorrowed: true, id_gt: "${last}" }) {
                    id
                    health
                    tokens(first: 10) {
                        symbol
    ...
    async setCompoundAccounts() {
        ...
        let blockNumber = await this.fetchLatestSyncedBlockNumber();
        let accounts = [];
    ...
    // 15% max deviation at every price update per block? UPDATE tweak this
