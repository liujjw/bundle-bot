...
class AccountsDbClient {
    ...
    constructor(db, provider) {
        ...
        this.anchoredPriceFeedContract = new ethers.Contract(ADDRS.OLD_UNISWAP_ANCHORED_VIEW, ABIS.OLD_UNISWAP_ANCHORED_VIEW, this.provider);
        this.compoundComptrollerContract = new ethers.Contract(ADDRS.COMPOUND_COMPTROLLER, ABIS.COMPOUND_COMPTROLLER, this.provider);
    ...
    async setCompoundAccounts(blockNumber=undefined) {
        ...
        let keys = await this.client.keys("account:*");
        multi.del(keys);
    ...
    async getStoredCompoundAccounts(hexDigit=undefined) {            
        ...
        let keys = await this.client.keys(keyQueryString);
        if(keys.length === 0)
            throw new Error("Accounts db empty");

        for(const key of keys) {
            let jsonStr = await this.client.get(key);
    ...
    */
    ...
    async getStoredCompoundParams() {
        let liquidationBonus = BigNumber.from(await this.client.get("liquidationBonus"));
        let closeFactor = BigNumber.from(await this.client.get("closeFactor"));
        let collateralFactors = JSON.parse(await this.client.get("collateralFactors"));
        let exchangeRates = JSON.parse(await this.client.get("exchangeRates"));
        let prices = JSON.parse(await this.client.get("prices"));
        for(let cf in collateralFactors) {
