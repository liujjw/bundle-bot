...
const { ENDPOINTS, ABIS, ADDRS, PARAMS } = require("./Constants");
...
class AccountsDbClient {
  ...
  constructor(db, provider=null) {
    ...
    this.aavePriceOracle = new ethers.Contract(
      ADDRS.AAVE_PRICE_ORACLE,
      ABIS.AAVE_PRICE_ORACLE,
      this.provider
    );
    this.aaveProtocolDataProvider = new ethers.Contract(
      ADDRS.AAVE_PROTOCOL_DATA_PROVIDER,
      ABIS.AAVE_PROTOCOL_DATA_PROVIDER,
      this.provider
    );
    this.aaveLendingPoolV2 = new ethers.Contract(
   ...
   */
  async setNonce(val) {
    await this.client.set("nonce", val.toString());
  }
   ...
   */
  async getNonce() {
    return Number.parseInt(await this.client.get("nonce"));
  }
   ...
   * 
   */
  async loadAaveTokenPrices() {
    // BigNumbers
    // 18 decimal places in eth wei units 
    // weth/eth is 1 ether obviously
    const prices = await this.aavePriceOracle.getAssetsPrices([
      ADDRS.WBTC,
      ADDRS.DAI,
      ADDRS.USDC
    ]);
   ...
   * 
   */
  async loadAaveReserveConfigData() {
    // BigNumber
    const wethData = await this.aaveProtocolDataProvider
      .getReserveConfigurationData(ADDRS.WETH);
    const wbtcData = await this.aaveProtocolDataProvider
      .getReserveConfigurationData(ADDRS.WBTC);
    const daiData = await this.aaveProtocolDataProvider
      .getReserveConfigurationData(ADDRS.DAI);
    const usdcData = await this.aaveProtocolDataProvider
      .getReserveConfigurationData(ADDRS.USDC);
    return {
      // seems scaled by 1e4
      // eg 8250
      ltvs: {
  ...
  async setAaveParams() {
    ...
    const prices = await this.loadAaveTokenPrices();
    const configData = await this.loadAaveReserveConfigData();

    for (const symbol in prices) {
      prices[symbol] = prices[symbol].toString();
    }

    for (const type in configData) {
      for (const symbol in configData[type]) {
        configData[type][symbol] = configData[type][symbol].toString();
      }
    }

    await this.client.set("aavePrices", JSON.stringify(prices));
    await this.client.set("aaveltvs", JSON.stringify(configData.ltvs));
    await this.client.set("aaveLiquidationBonuses", 
      JSON.stringify(configData.liquidationBonuses));
    await this.client.set("aaveLiquidationThresholds", 
      JSON.stringify(configData.liquidationThresholds));

    // HARDCODED and scaled by 1e4
    await this.client.set("aaveCloseFactor", "5000");
  }
   ...
   * @param {*} blockNumber 
   */
  async fetchBatchOfAaveAccounts(last, blockNumber) {
    const queryString = `query Accounts {
      _meta {
        deployment
      }
      users(first: 1000, where: { id_gt: "${last}" }) {
        id
      }
    }`;

    const res = await fetch(ENDPOINTS.AAVE_SUBGRAPH, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ query: queryString }),
    });
    const obj = await res.json();
    if (obj.data.users.length == 0) return null;
   ...
   * @param {*} minHealth 
   */
  async setAaveAccounts(
    blockNumber = undefined,
    maxHealth = PARAMS.AAVE_USER_MAX_HEALTH,
    minHealth = PARAMS.AAVE_USER_MIN_HEALTH
  ) {
    const totalSickAccounts = 0;
    let totalBatches = 0;

    let lastID = "";
    while (true) {
      const batch = await this.fetchBatchOfAaveAccounts(lastID, blockNumber);
      ...
      for (const user of batch) {
        ...
        // everything else is scaled by 1e18
        const userData = 
          await this.aaveLendingPoolV2.getUserAccountData(user.id);
        const healthFactor = userData[5];
        maxHealth = BigNumber.from(maxHealth);
        minHealth = BigNumber.from(minHealth);
        if (healthFactor.lte(maxHealth) && healthFactor.gte(minHealth)) {
          console.log(user.id);
          process.exit(0);
          // TODO add the user to the db
          // add its markets entered to know what asset to seize/repay
          // does graphql tell us or does eth_call tell us?
          // spin up forked anvil chains to simulate
          // use getUserReserveData, the subgraph sucks
        }
      }
      
    }

    await this.client.set("totalAaveBatches", totalBatches);
  }
  ...
  async fetchBatchOfAccounts(last, blockNumber) {
    ...
    const queryString = `query {
                accounts(block: {number: ${blockNumber}}, first: 1000, where: { hasBorrowed: true, id_gt: "${last}" }) {
                    id
                    health
                    tokens(first: ${PARAMS.COMPOUND_NUM_TOKENS_PER_ACCOUNT}) {
                        symbol
  ...
  async setCompoundAccounts(
    ...
    maxHealth = PARAMS.COMPOUND_USER_MAX_HEALTH,
    minHealth = PARAMS.COMPOUND_USER_MIN_HEALTH
  ) {
    ...
    while (true) {
      ...
      for (const user of batch) {
        if (!ADDRS.BLACKLISTED_USERS.includes(user.id.toLowerCase())) {
    ...
    // not really necessary, since the low accs are spread out over batches and
    // olds will get deleted  
    // let incr = 0;
    // do {
    //   if (await this.client.scard(totalBatches + incr) !== 0) {
    //     await this.client.del(totalBatches + incr);
    //     incr++;
    //   } else {
    //     break;
    //   }
