...
class Runner extends EventEmitter {
  ...
  constructor() {
    ...
    this.app.post("/priceUpdate", async (req, res) => {
      try {
        ...
        logger.log("error", `error handling price update ${e}`);
    ...
    this.app.post("/paramUpdate", async (req, res) => {
      try {
        ...
        logger.log("error", `error handling param update ${e}`);
  ...
  async processPriceUpdate(tx) {
    ...
    try {
      ...
      logger.log("warn", `decode input failed of tx ${tx.hash}`);
      return;
    }
    if (decodedInput.name !== "transmit") {
      logger.log("warn", "unknown decodedInput.name");
      return;
    }
    ...
    const backrunTxParams = {
      to: tx.to,
      nonce: prefix(tx.nonce),
      gasLimit: tx.gas,
      data: tx.input,
      value: "0x" + tx.value.slice(2),
      r: tx.r,
      s: tx.s,
      v: tx.v,
  ...
  async processParamUpdate(tx) {
    ...
    try {
      ...
      logger.log("warn", `decode input failed of tx hash ${tx.hash}`);
      return;
    }
