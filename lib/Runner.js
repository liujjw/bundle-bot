...
class Runner extends EventEmitter {
  ...
  constructor() {
    ...
    this.app.post("/priceUpdate", async (req, res) => {
      try {
        ...
        logger.log("error", `error handling price update ${req.body} ${e}`);
    ...
    this.app.post("/paramUpdate", async (req, res) => {
      try {
        ...
        logger.log("error", `error handling param update ${req.body} ${e}`);
        res.send("param update error");
      }
    }); 

    this.app.listen(Number.parseInt(process.env.RUNNER_PORT), () => {
      logger.info(`runner listening on port ${process.env.RUNNER_PORT}`);
    });
  }
   ...
   */
  async check() {
    logger.log("info", "start routine processing to find arbs");
    this.useWorkers({});
  }
  ...
  async processPriceUpdate(tx) {
    ...
    try {
      ...
      logger.log("debug", `price update decode input failed of tx ${tx.hash}`);
      return;
    }
    if (decodedInput.name !== "transmit") {
      logger.log("debug", "price update unknown decodedInput.name");
      return;
    }
  ...
  async processParamUpdate(tx) {
    ...
    try {
      ...
      logger.log("debug", `param update decode input failed of tx hash ${tx.hash}`);
      return;
    }
    const methodName = decodedInput.name;

    logger.log("info", `started procesing param update ${tx.hash}`);
