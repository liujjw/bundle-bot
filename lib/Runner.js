...
class Runner {
    constructor() {
        ...
        this.app.post('/priceUpdate', async (req, res) => {  
            try {
                ...
                this.logger.error(`handling /priceUpdate ${e} with stack trace: \n ${e.stack}`);
            }
            res.send("done");
        });
    ...
    async processPriceUpdate(tx) {
        ...
        this.logger.info(`started processing pending price update with hash ${tx.hash}`);
        ...
        let unserializedBackrunTx = FeeMarketEIP1559Transaction.fromTxData(backrunTxParams, { common });
        ...
        this.logger.info(`using data to find liquidation:\n ${JSON.stringify(finderData)}`);
    ...
    useWorkers(finderData) {
        ...
        for(let i = 0; i < this.splitFactor; i++) {
            ...
            piscina.run(finderData).catch(e => { throw e });
        }
    }
}
