...
class FindShortfallPositions {
    ...
    setParam(type, value) {
        if(type === "closeFactor") {
            this.closeFactor = Utils.bigNumToFloat(value, 18);
        } else if(type == "liquidationIncentive") {
            this.liquidationIncentive = Utils.bigNumToFloat(value, 18);
        } else if(type == "prices") {
            this.prices[value.ticker] = Utils.bigNumToFloat(value.newPrice, 8);
        }
    }
