...
class FindShortfallPositions {
  ...
  /**
   * 
   * @param {bool} isIterative 
   * @param {bool} isIterativeNew
   * @return {LiquidationTx[]}
   */
  async getLiquidationTxsInfo(isIterative=false, isIterativeNew=false) {
    if (isIterative) {
      if (isIterativeNew) {
        this.accountsOffset = 0;
      }
      const trimmedAccounts = this.accounts.slice(this.accountsOffset);
      for (const [idx, user] of trimmedAccounts.entries()) {
        try {
          const liquidationTx = await this.getLiquidationTxInfo(user);
          if (liquidationTx !== null) {
            this.accountsOffset = idx + 1;
            return [liquidationTx];
          }
        } catch (e) {
          logger.warn(`problem processing user id ${user.id} ${e}`);
        }
      }
      return [];
    } else {
