...
const { underlying, parseUnits, bigNumToFloat, bigNumsToFloats, 
  bigDecToFloat, numDecimals }= require("./Utils");
...
class FindShortfallPositions extends EventEmitter {
  ...
  async getLiquidationTxsInfo() {
    ...
    for (const user of this.accounts) {
      try {
        ...
        this.emit("error", new Error(`processing user id ${user.id} 
        with message ${e} and stack trace ${e.stack}`));
      }
    }
  ...
  async getLiquidationTxInfo(user) {
    ...
    const repayAmountBase = parseUnits(
    ...
    const exchangeRateScaleFactor = BigNumber.from(10).pow(
      numDecimals(underlying(cTokenToSeizeSymbol))
    );
    const exchangeRate = exchangeRateBase.div(exchangeRateScaleFactor);
    const tokensSeizedBase = exchangeRate.mul(cTokensSeizedBase);
    ...
    const weiGasCostToLiquidate = gasUpperBound.mul(this.gasPrice);
