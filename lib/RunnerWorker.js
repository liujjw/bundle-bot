...
let provider = (process.env.NODE_ENV !== "production") ? new ethers.getDefaultProvider('goerli') : new ethers.providers.JsonRpcProvider(process.env.PROVIDER_ENDPOINT);
...
async function bundleAndSendTxsToFlashbots(liquidationTxsInfo, backrunTxs) {
    let flashbotsProvider = (process.env.NODE_ENV !== "production") ? 
    await FlashbotsBundleProvider.create(signerWithProvider.provider, flashbotsAuthSigner, 'https://relay-goerli.flashbots.net/', 'goerli') : 
    await FlashbotsBundleProvider.create(signerWithProvider.provider, flashbotsAuthSigner); 

    for (let txInfo of liquidationTxsInfo) {
        ...
        let signedTransactionsBundle = await flashbotsProvider.signBundle(bundle);
        ...
        if('error' in bundleSubmission) {
            logger.error(`Bundle submission error: ${bundleSubmission.error.message}`);
            return;
        }
...
module.exports = async (finderData) => {
    ...
    await store.init();
    finderData.newParamValue.value = BigNumber.from(finderData.newParamValue.value);
    ...
    let finder = new FindShortfallPositions(
        ...
        params,
        lowerBoundGasPrice,
        (process.env.NODE_ENV !== "production") ? new ethers.providers.JsonRpcProvider(process.env.PROVIDER_ENDPOINT) : provider 
    );
    ...
    let result = await finder.getLiquidationTxsInfo();
