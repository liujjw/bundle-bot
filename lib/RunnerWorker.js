...
class RunnerWorker {
   ...
   */
  async simulate(signedTransactionsBundle) {
    const simulation = await this.flashbotsProvider.simulate(
      signedTransactionsBundle,
      this.curBlockNumber + this.futureBlocks
    );
    if ("error" in simulation) {
      logger.debug(`Flashbots simulation error
  ...
  async send(signedTransactionsBundle) {
    ...
    logger.info(`submitted signedTxBundle at ${this.curBlockNumber}`); 
    logger.verbose(`submitted signedTxBundle at ${this.curBlockNumber}
    ...
    bundleSubmission.wait().then(async (waitResponse) => {
      ...
      logger.info(`Wait response for bundle at ${this.curBlockNumber}: ${str}`);
  ...
  async constructBundle(liquidationTxsInfo, backrunTxs=undefined) {
    ...
    for (const txInfo of liquidationTxsInfo) {
      ...
      //   .then((foo) => {
      //     logger.info(`batchcall price simulation success ${foo}`);
      //   });
      ...
      //     common
      // ).then((foo) => {
      //   logger.info(`batchcall simulation success ${foo}`);
      // }).catch((err) => {
      //   logger.error(`batchcall simulation ${err}`);
      // });
      //
    ...
    logger.verbose(`bundle of raw backrun tx and liquidations for ${this.curBlockNumber}:
      ${bundle}`);
