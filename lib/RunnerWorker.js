 ...
 */
  async bundleAndSendTxsToFlashbots(liquidationTxsInfo, backrunTxs) {
    ...
    for (const txInfo of liquidationTxsInfo) {
      const mevInfo = await this.bidPricer.getTip(
        BigNumber.from(PARAMS.COMPOUND_LIQ_GAS_USED),
      ...
      if ("error" in simulation) {
        if (process.env.NODE_ENV === "production") {
          this.emit("info", `Flashbots simulation error: 
  ...
  async process(finderData) {
    ...
    await this.bundleAndSendTxsToFlashbots(result, finderData.backrunTxs);
  };
}
