...
class RunnerWorker extends EventEmitter {
  ...
  constructor() {
    ...
    this.signerWithProvider = new ethers.Wallet(process.env.MM0A_PK, this.provider);
    ...
    this.flashbotsAuthSigner = new ethers.Wallet(process.env.MM0A2_PK);
    this.futureBlocks = 2;
  ...
  async bundleTx(liquidationTxsInfo, backrunTxs) {
    for (const txInfo of liquidationTxsInfo) {
      ...
      const params = ABIS.botEncode(
        ...
        mevInfo.weiTip,
        mevInfo.weiTip.add(
          ethers.utils.parseEther(PARAMS.MIN_LIQ_PROFIT_IN_ETH)
        )
      );
      ...
      const signedTransactionsBundle = await this.flashbotsProvider.signBundle(bundle);
      ...
      if ("error" in simulation) {
        ...
        if (simulation.results[0].error === "execution reverted") {
          ...
          console.log("executing reverted detected by flashbots sim, continuing");
          // await this.store.decrNonce();
          // return;
        }
      }
