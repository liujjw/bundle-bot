...
async function bundleAndSendTxsToFlashbots(liquidationTxs, backrunTx) {
    ...
    for (let tx of liquidationTxs) {
        ...
        logger.log(`for adjusted gas price of ${ethers.utils.formatUnits(gasPriceAndReward.bundleAdjustedGasPrice, 9)} gwei`); 
...
module.exports = async (finderData) => {
    ...
    finderData.newParamValue.newPrice = BigNumber.from(finderData.newParamValue.newPrice._hex);
    ...
    if(finderData.newParamValue.ticker === "ETH")
        bidPricer.ethPrice = Utils.bigNumToFloat(finderData.newParamValue.newPrice, 8);

    /// TEST 
    // let scale = 1e8;
    // let newPrice = BigNumber.from(500 * scale);
    // finder.setParam("prices", { ticker: "ETH", newPrice: newPrice });
