...
exports.ADDRS = {
    ...
    OFFCHAIN_AGG: OFFCHAIN_AGG,
    REVERSE_OFFCHAIN_AGG: invert(OFFCHAIN_AGG)
};
...
exports.PARAMS = {
    ...
    POST_PRICES_GAS_UPPER_BOUND: 210000,
    GAS_USED_UPPER_BOUND: 1.3e6 + 210000,
    /* With most of profits going to miners, will need profit to be low, but not so low that math/price imprecision could give negative profit. Scenario: 20k * 1.1 = 2k profit -> 90% go to miner -> 2k - 1.8k -> leaves 200$ profit. It follows that only 20k collateral liquidations will be profitable. We also do not want to start giving miners over 90% of profit. Imagine a more gas efficient liquidator also pays 1.8k in fees, but they spend, say 500k gas instead of my bot spending 1 million gas. Then they have double the profit per gas. And higher payment per gas always wins. Thus it will be hard to compete with more gas efficient, e.g. non-flashloan based bots. But maybe its still possible to win sometimes. */
    MIN_LIQ_PROFIT: 200,
    DB_UPDATE_ACCOUNTS_SLEEP_MS: 20000,
    DB_UPDATE_PARAMS_SLEEP_MS: 10000,
    ELAPSED_TIME_TO_PROCESS_DB_MS: 8000,
    WAIT_TIME_FOR_DB_TO_INIT_MS: 80000 
}