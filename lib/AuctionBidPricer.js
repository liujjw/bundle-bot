...
class AuctionBidPricer {
  ...
  async v1_getWinningGasPrice(
    ...
    for (let block of blocks) {
      ...
      let gasFeeInEth = Number.parseFloat(
  ...
  async v2_getWinningGasPrice(gasUsedUpperBound, upperBoundProfit) {
    let mevRatio = PARAMS.v2_WINNING_GAS_PRICE_FRACTION_TO_PAY_MINER;
    let mevPaymentDollars = upperBoundProfit * mevRatio;
    let mevPaymentWei = ethers.utils.parseEther(
