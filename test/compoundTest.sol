...
contract CompoundTest is Test {
    ...
    address cUSDC_ADDR = 0x39AA39c021dfbaE8faC545936693aC917d5E7563;
    address cWBTC_ADDR = 0xC11b1268C1A384e55C48c2391d8d480264A3A7F4;
    ...
    function xtestLiquidate2() public {
        ...
        // TODO make the blocktimetime 15 blocks earlier to fix this
    ...
    function xtestFlashLoan() public {
    ...
    /**
    * @notice Collateral is related to Eth.
    */
    // reverts for no reason
    function xtestEthCollateralSwap() public {
        // Long eth(weth), borrowed DAI.
        // Thus get eth to convert to DAI repay flashloan.
        uint numWeth = 100 * 10**18;
        uint slippage = 3 * 10**18;
        uint numDai = 100_000 * 10**8;
        uint minerPayment = 1 * 10**17;
    ...
    /**
    * @notice Both collateral and borrowed are not eth. 
     */
    function testErc20CollateralSwap() public {
        // Long WBTC, borrowed USDC.
        uint numWBTC = 4 * 1e8;
        uint slippage = 5 * 1e7;
        uint numUSDC =  100_000 * 1e6;
        uint minerPayment = 1 * 1e17;

        // has 9.6 wbtc
        vm.prank(WBTC_ADDR);
        Erc20Interface(WBTC_ADDR).transfer(address(compoundBot), numWBTC + slippage);
        assertEq(Erc20Interface(WBTC_ADDR).balanceOf(address(compoundBot)), numWBTC + slippage);
